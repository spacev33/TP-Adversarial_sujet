...
def plot_results(losses_train, losses_test, acc_train, acc_test, args):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(
        np.arange(len(losses_train)) / len(losses_train) * (len(losses_test) - 1),
        losses_train,
        label="Train",
    )
    ...
    plt.subplot(1, 2, 2)
    plt.plot(
        np.arange(len(acc_train)) / len(acc_train) * (len(acc_test) - 1),
        acc_train,
        label="Train",
    )
    ...
    for i in range(5):
        for j in range(5):
            ...
            title = ax[i][j].set_title(
                "Pred: {}".format(cifar10_classes[int(y_pred[i * 5 + j])])
            )
